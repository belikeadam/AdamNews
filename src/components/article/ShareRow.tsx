...
export default function ShareRow({ slug, title, categoryColor }: ShareRowProps) {
  ...
  const buttonClass = "h-9 flex items-center gap-1.5 px-3 rounded-lg border border-[var(--border)] text-xs font-medium text-[var(--muted)] hover:text-[var(--text)] hover:bg-[var(--surface)] transition-colors"

  return (
    <div className="flex flex-wrap items-center gap-2 mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-[var(--border)]">
      <span className="text-xs sm:text-sm font-medium text-[var(--muted)] mr-1">Share</span>
      ...
      >
        {copied ? (
          <>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--success)" strokeWidth="2" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
